...
class MyTest(unittest.TestCase):
    ...
    def test_01(self):
        ...
        self.assertEqual(200, getModelProfileByKeyUsingGET.status_code, "调用--查询优模型--接口失败。")
        ...
        if "utMsg"  in updateThingPropertyUsingPOST.text:
            raise ValueError("无法修改属性,状态码为: %s ,返回信息为：%s" % ((updateThingPropertyUsingPOST.status_code), updateThingPropertyUsingPOST.json()["utMsg"]))
        else:
            pass
        """查询属性"""
        ...
        if "utMsg"  in findThingPropertyUsingGET.text:
            raise ValueError("无法查询属性,状态码为: %s ,返回信息为：%s" % ((findThingPropertyUsingGET.status_code), findThingPropertyUsingGET.json()["utMsg"]))
        else:
            if  "修改属性之后" != findThingPropertyUsingGET.json()["description"]:
                logs.error("修改失败")
                raise ValueError("修改属性失败,未修改成功，状态码为: %s ,返回信息为：%s" % ((findThingPropertyUsingGET.status_code), findThingPropertyUsingGET.json()["description"]))
        """删除属性"""
        ...
        if "utMsg" in deleteThingPropertyUsingPOST.text:
            raise ValueError("无法删除属性,状态码为: %s ,返回信息为：%s" % ((deleteThingPropertyUsingPOST.status_code), deleteThingPropertyUsingPOST.json()["utMsg"]))
        else:
            pass
